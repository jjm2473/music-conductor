...
from app.metadata_service import read_metadata, update_metadata
from app.models import MetadataReadRequest, MetadataUpdateRequest
...
class MetadataServiceTests(unittest.TestCase):
    ...
    def test_read_metadata_returns_full_payload(self) -> None:
        with (
            patch("app.metadata_service.read_full_metadata", return_value=({"tags": {"title": ["B"]}}, None)),
            patch("app.metadata_service.read_duration_seconds", return_value=12.34),
        ):
    ...
    def test_update_metadata_collects_failure(self) -> None:
        with patch("app.metadata_service.write_easy_metadata", side_effect=ValueError("write not supported")):
    ...
    def test_update_metadata_success(self) -> None:
        with patch("app.metadata_service.write_easy_metadata", return_value=None):
            result = update_metadata(
                MetadataUpdateRequest(
                    directory=str(self.directory),
                    file_name=self.file_name,
